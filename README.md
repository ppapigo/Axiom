...
현재 Unity EditMode 테스트는 35개이며 모두 통과합니다.
...
- Assassin: HP 900, Attack 115, 이동 배율 1.10, 8m 회피, 5초 쿨다운
- Assassin의 Inspector 기반 광역 반경 제한

## Unity 구성

1. `Create > Axiom > Character > Movement Profile`로 이동 프로필을 생성합니다.
2. `Create > Axiom > Role` 메뉴에서 Tank, Mage 또는 Assassin 역할 데이터를 생성합니다.
...
State Machine 기반 AI를 구현합니다. Idle, FindTarget, Move, Attack, UseSkill, Retreat, Dead 상태와 역할별 타깃 선택 정책을 분리합니다.
