...
Unity EditMode 테스트 155개와 최신 데모 PlayMode 테스트 2개를 구성했습니다. 속성 제작·AI 검증 관련 테스트 87개는 Unity 배치 모드에서 통과했습니다.
...
- Vercel Production 함수 `https://axiom-skill-api.vercel.app/api/generate-skill` 배포 및 데모 씬 설정 연결
...
- 기본 공격·피격·스킬 시전·폭발 효과음을 런타임 파형으로 생성해 별도 오디오 에셋 없이 재생
- 피격 시 졸라맨 전체가 짧게 점멸하고 누적 피해 숫자가 위로 떠오르는 무에셋 타격 피드백
- 역할별 핵심 수치와 경기 규칙을 보여주는 시작 화면, 라운드 배너와 최종 승패 오버레이
- 역할 선택 → Q/E/R 제작 → 외형 아이템 선택 → 3vs3 시작으로 이어지는 제출용 준비 흐름
...
6. 데모는 `역할 선택 → Q 제작·저장 → E 제작·저장 → R 제작·저장 → 3vs3 전투 시작` 순서로 진행됩니다. 세 스킬을 모두 저장하기 전에는 전투 캐릭터와 매치가 생성되지 않습니다. 각 슬롯은 별도의 피해·범위·사거리·쿨타임·공격 방식·CC·속성 설정을 보존합니다. 한 스킬에는 속성을 최대 1개만 부여할 수 있고, Q/E/R 중 속성을 가진 스킬은 최대 2개입니다. Mage는 모든 속성, Tank는 바람·대지, Assassin은 독·번개만 사용할 수 있습니다.
...
29. 속성 규칙을 역할 단위로 통합했습니다. 각 스킬은 속성 최대 1개, Q/E/R 중 속성 스킬은 최대 2개이며 Mage는 전체 속성, Tank는 바람·대지, Assassin은 독·번개만 선택할 수 있습니다. 수동 제작, AI 검증·자동 보정, 기본 프리셋과 실제 전투가 같은 규칙을 사용합니다.
30. 피해 숫자 표시 종료 시각의 부동소수점 오차를 보정했습니다. 표시 종료점과 거의 같은 시간은 만료로 처리하고 종료 직전 프레임은 유지하므로, 정확한 경계에서도 한 프레임 더 남는 현상이 없습니다.
31. Classic, Obsidian, Ivory 장비 키트가 색상뿐 아니라 실루엣으로도 구분됩니다. Obsidian은 크고 각진 방패·결정형 지팡이·긴 단검, Ivory는 둥근 방패·후광 지팡이·넓은 단검을 Unity 기본 도형만으로 조합하며 외부 모델이나 텍스처 용량을 추가하지 않습니다.
32. 경기 종료 화면에서 `REMATCH`는 현재 역할·Q/E/R·장비를 유지한 채 3판 2선승을 다시 시작하고, `NEW LOADOUT`은 데모 씬을 깨끗하게 다시 불러 역할 선택부터 새 스킬을 제작할 수 있게 합니다.
...
GitHub Pages에서 역할 선택부터 자연어 스킬 생성, 장비 선택, 3vs3 전투까지 전체 제출 흐름을 최종 리허설하고 심사 영상과 제출 설명을 준비합니다.
